...
function TablePatientsWithNestedStudies({
    ...
    //change const to var by rishabh
    var data = useMemo(() => studyArrayToPatientArray(studies).map(patient => {
    ...
    const columns = useMemo(() => [
        ...
        patientColumns.NAME(),
        patientColumns.ID(),
        patientColumns.OtherPatientIDs,
        patientColumns.ReferringPhysicianName,
        ...(!hiddenActionBouton ? [patientColumns.ACTION(onDeletePatient, onModify, refresh, true)] : []),
    ...
    return <NestedTable columns={columns} data={data} setSelected={setSelected} hiddenSelect={hiddenSelect}
        rowStyle={rowStyle}
        hide0and4={true} //added by rishabh
        rowEvent={(row) => {
