...
class OHIFCornerstoneViewportOverlay extends PureComponent {
  ...
  render() {
    ...
    let orientationDot = null;
    try {
      const { rowCosines, columnCosines } = imagePlaneModule;
      if (rowCosines && columnCosines && rowCosines.length === 3 && columnCosines.length === 3) {
        ...
        let orientation;
        // Axial: normal ≈ Z axis
        if (absNz >= absNx && absNz >= absNy) orientation = { label: 'A', color: '#ef4444' };
        // Sagittal: normal ≈ X axis
        else if (absNx >= absNy) orientation = { label: 'S', color: '#22c55e' };
        // Coronal: normal ≈ Y axis
        else orientation = { label: 'C', color: '#eab308' };
        if (orientation) {
          orientationDot = (
            <div
      ...
      // Orientation detection is cosmetic — silently ignore errors
    }
