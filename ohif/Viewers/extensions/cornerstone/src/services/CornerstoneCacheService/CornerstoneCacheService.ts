...
class CornerstoneCacheService {
  ...
  private _getCornerstoneVolumeImageIds(displaySet, dataSource): string[] {
    ...
    // PUTRACNS: this scanner stores every slice position twice (duplicate
    // ImagePositionPatient pairs), so the raw stack has 2x the unique slices.
    // A volume built from the raw stack gets a halved z-spacing (anatomically
    // squashed along the scan axis). Dedupe by rounded position so the volume
    // spacing/origin/dimensions reflect the true anatomy. The stack viewport
    // keeps all slices — this only affects volume construction.
    const seen = new Set<string>();
    const deduped: string[] = [];

    for (const imageId of stackImageIds) {
      const plane = metaData.get('imagePlaneModule', imageId);
      const pos = plane?.imagePositionPatient;
      if (!pos) {
        deduped.push(imageId); // no position -> cannot dedupe, keep
        continue;
      }
      const key = pos.map(v => Math.round(v * 100) / 100).join(',');
      if (!seen.has(key)) {
        seen.add(key);
        deduped.push(imageId);
      }
    }

    return deduped.length ? deduped : stackImageIds;
  }
}
