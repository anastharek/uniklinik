...
function commandsModule({
  ...
  const actions = {
    ...
    /**
     * PUTRACNS: warms the browser image cache for a series (or a whole study)
     * by loading every frame through the normal cornerstone pipeline, so the
     * decoded images land in the CS3D cache and later scrolling is
     * near-instant. Meant to run AFTER the server-side preload
     * (/api/preload) has warmed Orthanc — frames then transfer fast and land
     * in the local cache. Frames already cached resolve instantly (no
     * re-download); per-frame failures are tolerated.
     */
    warmDisplaySetCache: async ({
      seriesInstanceUID,
      studyInstanceUID,
      onProgress,
      concurrency = 4,
      ...
      let displaySets = seriesInstanceUID
      ...
      // Warm biggest series first: the CS3D cache is LRU, so the last-loaded
      // frames survive — the small review series end up cached and the large
      // ones (which wouldn't fit anyway) get evicted from their tail.
      displaySets = [...displaySets].sort((a, b) => (b.numInstances || 0) - (a.numInstances || 0));

      const imageIds: string[] = [];
      const seen = new Set<string>();
      const seriesSet = new Set<string>();
      const seriesFirstImageId = new Map<string, string>();
      let anyStudyInstanceUID = studyInstanceUID;

      for (const ds of displaySets) {
        if (!ds?.SeriesInstanceUID) continue;
        anyStudyInstanceUID = anyStudyInstanceUID || ds.StudyInstanceUID;
        seriesSet.add(ds.SeriesInstanceUID);
        let ids: string[] = [];
        try {
          ids = dataSource.getImageIdsForDisplaySet(ds);
        } catch (e) {
          /* display set types that can't resolve image ids */
        }
        if (!ids?.length && ds.images?.length) {
          ids = ds.images.map(i => i.imageId).filter(Boolean);
        }
        for (const id of ids) {
          if (!seen.has(id)) {
            seen.add(id);
            imageIds.push(id);
            if (!seriesFirstImageId.has(ds.SeriesInstanceUID)) {
              seriesFirstImageId.set(ds.SeriesInstanceUID, id);
            }
          }
        }
      }
      ...
      // Decoding needs full instance metadata. If the study was never opened
      // in this session, fetch it per series via the data source first.
      if (anyStudyInstanceUID) {
        for (const seriesUid of seriesSet) {
          const sampleId = seriesFirstImageId.get(seriesUid);
          if (sampleId && metaData.get('imagePlaneModule', sampleId)) {
            continue; // metadata already available
          }
          try {
            await dataSource.retrieve.series.metadata({
              StudyInstanceUID: anyStudyInstanceUID,
              filters: { seriesInstanceUID: seriesUid },
            });
          } catch (e) {
            /* tolerate — frames without metadata will fail individually */
          }
        }
      }
      ...
      let loaded = 0;
      let idx = 0;
      const n = Math.max(1, Math.min(concurrency, imageIds.length));
      const worker = async () => {
        while (idx < imageIds.length) {
          const imageId = imageIds[idx++];
          try {
            await imageLoader.loadImage(imageId);
          } catch (e) {
            console.warn('[warm] frame load failed', imageId, e);
          }
          loaded += 1;
  ...
  const definitions = {
    ...
    warmDisplaySetCache: actions.warmDisplaySetCache,
  };
