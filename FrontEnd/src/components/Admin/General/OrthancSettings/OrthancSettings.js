...
export default class OrthancSettings extends Component {
    ...
    testConnexion = () => {
        apis.options.getOrthancSystem().then((answer) => {
            toast.success('PadiMedical Version: ' + answer.Version)
        }).catch(error => { toast.error(error.statusText) })
    }
    ...
    shutdown = () => {
        apis.options.shutdownOrthanc()
            .then(() => { toast.success('PadiMedical Stopped') ; this.setState({ showShutdown: false })})
            .catch((error) => {
                toast.error(error.statusText)
            })
    }
        ...
        return (
            <Fragment>
                <div className="form-group">
                    <Row>
                        <Col>
                            <h2 className="card-title">PadiMedical Server</h2>
                        </Col>
                        <Col className="text-center">
                            ...
                            <Modal show={this.state.showOrthancDetails} onHide={() => this.setState({ showOrthancDetails: false })}>
                                <Modal.Header closeButton>
                                    <Modal.Title>PadiMedical Details</Modal.Title>
                                </Modal.Header>
                                <Modal.Body>
                                    <OrthancInfos/>
                                </Modal.Body>
                ...
                <Row className="mt-5 text-center">
                    ...
                    <Col sm={4}>
                        ...
                        <Modal show={this.state.showRestart} onHide={() => this.setState({ showRestart: false })}>
                            ...
                            <Modal.Body>Are you sure to restart PadiMedical system ?</Modal.Body>
                            <Modal.Footer>
                    ...
                    <Col sm={4}>
                        ...
                        <Modal show={this.state.showShutdown} onHide={() => this.setState({ showShutdown: false })}>
                            ...
                            <Modal.Body>Are you sure to shutdown PadiMedical system ?</Modal.Body>
                            <Modal.Footer>
