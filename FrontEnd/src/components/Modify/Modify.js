...
export default class Modify extends Component {
    ...
    constructor(props){
        super(props);
        this.state.dataRows=this.props.row;
    }
    state = {
        prevStudy:[],
        newStudy:[],
    ...
    componentDidMount(){
        ...
        fetch(`/api/${this.props.level||'studies'}/${this.props.orthancID}`, {
            method: "GET",
            headers: {
          ...
          .then(resJson=>{
            this.setState({dataRows:resJson?.MainDicomTags||[]})
          }).catch(()=>{})
    }
    ...
    openModify = () => {
        ...
        for (let tag in this.state.dataRows) {
            if (!forbidden.includes(tag))
                rows.push(
                    {
                        'TagName': tag,
                        'Value':
                           this.state.dataRows[tag] ?
                           this.state.dataRows[tag] : '',
                        deletable: false
                    })
        }
    ...
    sync=async(data)=>{
        if(this.props.level=='patients'){
            let patientNewData=await apis.content.getPatientDetails(data.Content.PatientID)
            apis.content.syncStudieIdWithReportBulk(this.state.prevStudy,patientNewData.Studies)
        }
        else if(this.props.level=='studies'){
            apis.content.syncStudieIdWithReport(this.props.orthancID,data.Content.ID)
        }
    }
    ...
    modify = async () => {
        ...
        switch (this.props.level) {
            case 'patients':
                if (!this.state.modification.PatientID || this.state.modification.PatientID === '')
                    ...
                    let patientData=await apis.content.getPatientDetails(this.props.orthancID)
                    let studyData=await  Promise.all([...patientData.Studies.map((id)=>apis.content.getStudiesDetails(id))])
                    this.setState({prevStudy:studyData})
        ...
        if (jobAnswer !== '') {
            ...
            jobMonitoring.onFinish(async function (state,data) {
    ...
    render = () => {
        ...
        if (this.props.hidden !== true) {
            render = <Fragment>
                <button className='dropdown-item bg-green' type='button' onClick={this.openModify}>Modify</button>
                <ModalModify
